...
$key = Get-UninstallRegistryKey -SoftwareName 'PDFGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'PDFGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for PDFGeek. Remove it manually rather than guessing:"
